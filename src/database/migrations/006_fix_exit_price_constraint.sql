-- Migration: 006_fix_exit_price_constraint.sql
-- Description: Ensure exit_price column exists and positions_status_check constraint is properly set up

-- Add exit_price column if it doesn't exist
ALTER TABLE positions 
ADD COLUMN IF NOT EXISTS exit_price DECIMAL(20,8);

-- Ensure closed_at column exists 
ALTER TABLE positions 
ADD COLUMN IF NOT EXISTS closed_at TIMESTAMP;

-- Update any existing closed positions to have the required fields
-- This ensures no constraint violations when adding the constraint
UPDATE positions 
